...
def PatchVendor(info):
  ...
  info.script.AppendExtra('run_program("/sbin/sed", "-i", "s/wait,check,quota/latemount,wait,check,quota/", "/vendor/etc/fstab.mt6768");')
  info.script.AppendExtra('run_program("/sbin/sed", "-i", "s/fstab.mt6768$/fstab.mt6768 --early\\n    mount_all \/vendor\/etc\/fstab.mt6768 --late/", "/vendor/etc/init/hw/init.mt6768.rc");')
